...
## Can you predict throughput to maximise performance in coordinated Wi-Fi networks?


Link to Zindi: https://zindi.africa/competitions/next-gen-wifi-throughput-prediction-challenge

Watch a presentation on the problem statement [here](https://aiforgood.itu.int/event/machine-learning-for-throughput-prediction-in-multi-ap-wi-fi-networks-using-coordinated-spatial-reuse/).
...
In this challenge, you need to use machine learning to predict the throughput (performance) that a subset of APs can achieve in a c-SR network. This will allow network devices to derive the optimal set of groups in WLAN settings.

We have provided a synthetic dataset with data from multiple random WLAN deployments. The dataset contains:

the RSSI overheard by all stations in a neighbourhood
the subset of APs selected to collaboratively transmit in a given transmission opportunity (TXOP) using c-SR
the achieved throughput.
The goal is to build a model to predict the achievable throughput given arandom combination of APs that aim to transmit simultaneously.
...
### Evaluation
The error metric for this competition is [Root Mean Squared Error](https://zindi.africa/learn/zindi-error-metric-series-what-is-root-mean-square-error-rmse).

For every row in the dataset, submission files should contain 2 columns.

Only successful connections will be scored. I.e if TXOP is 0 then there will be no Throughput.

Please note that it can take up to 5 minutes to score the large submission file.

Your submission file should look like this:

| Deployment_x-TXOP_x-AP_x-STA_x        | Throughput |
|---------------------------------------|------------|
| Deployment_1-TXOP_1-AP_1-STA_1        | 75         |
| Deployment_1-TXOP_1-AP_1-STA_2        | 12         |
...
There are 2000 Zindi points for this challenge. You can read more about Zindi points [here](https://zindi.africa/discussions/13959?utm_source=zindi&utm_medium=blog&utm_campaign=challenge_resources&utm_id=CR).
